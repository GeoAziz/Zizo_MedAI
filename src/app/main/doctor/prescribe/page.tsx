...
import { getPatientsForDoctor } from '@/services/users';
...
export default function DoctorPrescribePage() {
  ...
  const patientIdFromQuery = searchParams ? searchParams.get('patientId') : null;
  const { user } = useAuth();
  ...
  useEffect(() => {
    async function fetchPatients() {
      ...
      try {
        if (!user?.uid) throw new Error("No doctor UID");
        const assignedPatients = await getPatientsForDoctor(user.uid);
  ...
  const onSubmit: SubmitHandler<PrescriptionFormValues> = async (data) => {
    ...
    if (!patients.find(p => p.uid === data.patientId)) {
      toast({ title: "Error", description: "You are not assigned to this patient.", variant: "destructive" });
      return;
    }
  ...
  return (
    <main className="flex-1 w-full px-2 md:px-6 lg:px-12 xl:px-24 2xl:px-48 py-6">
      <PageHeader 
                      ...
                      <Select onValueChange={field.onChange} value={field.value} disabled={isLoadingPatients || patients.length === 0}>
                        ...
                        <SelectContent>
                          { !isLoadingPatients && patients.length === 0 ? (
                            ...
                            patients.map(p => (
                                <SelectItem key={p.uid} value={p.uid}>{p.name} ({p.uid.substring(0, 6)}...)</SelectItem>
                            ))
                          )}
                        </SelectContent>
                      </Select>
