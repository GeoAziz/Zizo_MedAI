...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    // Call backend update (mocked for avatar, notifications, wearables)
    const result = await updateUserAction(user?.uid || "admin_main", { name, email });
    setIsSaving(false);
    if (result.success) {
        ...
        <Card className="lg:col-span-2 shadow-xl rounded-xl">
          ...
          <CardContent className="p-6 space-y-6">
            <div className="flex items-center gap-6 mb-6">
              <Avatar>
                {avatar ? <AvatarImage src={avatar} alt="avatar" /> : <AvatarFallback>MA</AvatarFallback>}
              </Avatar>
              <div>
                <label htmlFor="avatar-upload" className="block text-sm font-medium text-foreground mb-1">Change Avatar</label>
                <input id="avatar-upload" type="file" accept="image/*" onChange={handleAvatarChange} className="block" />
              </div>
            </div>
